...
// 1. Remove old isUpdate logic
code = code.replace(/    if \(isUpdate\) \{\s+const revEl = document\.getElementById\('reviews-section'\);\s+if \(revEl\) revEl\.outerHTML = reviewsHtml;\s+const recEl = document\.getElementById\('recommendations-section'\);\s+if \(recEl\) recEl\.outerHTML = recommendationsHtml;\s+return;\s+\}/g, '/* isUpdate block removed */');

// 2. Define reviewsHtml and recommendationsHtml outside the innerHTML string.
const isUpdateInjection = `
...
// Insert the new logic before appMain.innerHTML = `
code = code.replace(/appMain\.innerHTML = \`/, isUpdateInjection + '\n\n    appMain.innerHTML = `');

// Now, replace the old sections inside appMain.innerHTML
// Replacing the old REVIEWS SECTION IIFE with ${reviewsHtml}
code = code.replace(/\$\\{\(\(\) => \{\s*\/\/\s*---\s*REVIEWS SECTION\s*---[\s\S]*?\}\)\(\)\}/, '${reviewsHtml}');

// Replacing the old Recommendations SECTION IIFE with ${recommendationsHtml}
code = code.replace(/\$\\{\(\(\) => \{\s*const currentCatId[\s\S]*?\}\)\(\)\}/, '<div id="recommendations-section">${recommendationsHtml}</div>');
