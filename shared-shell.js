export function mountSharedShell(page = 'shop') {
    ...
    <div id="cart-sidebar" class="fixed top-0 right-0 h-full w-[85%] md:w-[420px] bg-white z-[2001] translate-x-full transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] shadow-[-20px_0_60px_rgba(0,0,0,0.1)] flex flex-col"><div class="shared-side-head p-6 border-b border-gray-50 flex items-center justify-between bg-white/80 backdrop-blur-md sticky top-0 z-10"><div class="flex flex-col"><h2 class="shared-side-title text-[13px] font-black uppercase tracking-[0.15em] text-black">Your Cart</h2><p id="cart-sidebar-count" class="shared-side-sub text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">0 Items</p></div><button aria-label="Close cart" onclick="window.closeCartSidebar()" class="shared-side-close w-10 h-10 rounded-full bg-gray-50 text-gray-400 flex items-center justify-center hover:bg-black hover:text-white transition-all duration-300"><i class="fa-solid fa-xmark text-sm"></i></button></div><div id="cart-items-list" class="shared-side-items flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar"></div><div class="shared-side-foot p-5 border-t border-gray-50 bg-gray-50/50 space-y-3"><div class="flex items-center justify-between px-1"><span class="text-[10px] font-black uppercase tracking-widest text-gray-500">Total</span><span id="cart-sidebar-total" class="text-[18px] font-black text-black">0.00 AED</span></div><button id="cart-checkout-btn" onclick="window.location.href='cart.html'" class="w-full bg-black text-white py-4 rounded-2xl font-bold text-[10px] uppercase tracking-widest shadow-lg flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all" style="display:none"><i class="fa-solid fa-lock text-lg"></i> Proceed to Checkout</button></div></div>
