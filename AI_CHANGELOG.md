...
- **Bug Fix (Product Detail Page LCP)**: Drastically improved the Product Detail Page speed by:
  ...
  5. **Deferred Firebase Init**: Delayed the initialization of Firestore (`initializeFirestore`) by 6000ms. This prevents the `Write/channel` WebSocket from blocking the Lighthouse "Network dependency tree" for 31+ seconds.
