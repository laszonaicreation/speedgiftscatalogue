...
// 1. Remove the Reviews IIFE and replace with container
const revStart = content.indexOf('${(() => {\n        // --- REVIEWS SECTION ---');
const revStartFallback = content.indexOf('${(() => {\r\n        // --- REVIEWS SECTION ---');
const rStart = revStart !== -1 ? revStart : revStartFallback;

if (rStart !== -1) {
    const rEnd = content.indexOf('        })()}', rStart) + 13;
    content = content.substring(0, rStart) + '<div id="reviews-section-container"></div>' + content.substring(rEnd);
}

// 2. Remove the Recommendations IIFE and replace with container
const recStart = content.indexOf('${(() => {\n            const currentCatId');
const recStartFallback = content.indexOf('${(() => {\r\n            const currentCatId');
const cStart = recStart !== -1 ? recStart : recStartFallback;

if (cStart !== -1) {
    // We must find the OUTERMOST })()
    // It ends right before </div>`;
    const cEnd = content.indexOf('        })()}', cStart) + 13;
...
// Insert the logic before appMain.innerHTML
