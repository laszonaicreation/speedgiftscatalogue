    ...
    const reviewsHtml = (() => {
        ...
        <div id="reviews-section" style="margin-top: 5rem; padding-top: 4rem;" class="border-t border-gray-100 w-full max-w-3xl mx-auto">
            ...
            <div class="flex flex-col items-center text-center" style="margin-bottom: 45px;">
                <h3 class="font-black text-gray-900 tracking-wide uppercase" style="font-size: clamp(24px, 3vw, 32px); margin-bottom: 12px; line-height: 1.2;">Customer Reviews</h3>
                <div class="flex items-center gap-3 bg-gray-50 px-4 py-2 rounded-full">
                    <span class="text-xl font-bold text-gray-900">${avgRating}</span>
                    <div class="flex text-lg" style="color: #FBBC04;">
                        ${[1, 2, 3, 4, 5].map(i => `<i class="${i <= (count > 0 ? avgRating : 0) ? 'fa-solid' : 'fa-regular'} fa-star mr-[2px]"></i>`).join('')}
                    </div>
                    <span class="text-sm font-medium text-gray-500">(${count} ${count === 1 ? 'Review' : 'Reviews'})</span>
                </div>
            </div>

            <!-- Review Form -->
            <div class="bg-gray-50 rounded-3xl p-6 md:p-8 mb-10 w-full border border-gray-100 relative overflow-hidden group">
                <div class="absolute inset-0 bg-gradient-to-br from-gray-50 to-white opacity-50 pointer-events-none transition-opacity group-hover:opacity-100"></div>
                <form id="review-form" onsubmit="window.submitReview(event, '${product.id}')" class="flex flex-col gap-5 relative z-10">
                    <div class="flex flex-col md:flex-row gap-5">
                        <div class="flex-1">
                            <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-2 pl-4">Your Name</label>
                            <input type="text" name="reviewerName" required class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-[14px] outline-none focus:border-black focus:ring-1 focus:ring-black transition-all shadow-sm" placeholder="John Doe">
                        </div>
                        <div class="w-full md:w-auto">
                            <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-2 pl-4">Rating</label>
                            <select name="rating" class="w-full md:w-32 px-5 py-4 bg-white border border-gray-200 rounded-2xl text-[14px] outline-none focus:border-black focus:ring-1 focus:ring-black transition-all shadow-sm appearance-none cursor-pointer">
        ...
        <div id="recommendations-section" class="mt-20 pt-12 border-t border-gray-50">
            ...
            return `
                    ...
                    return '<p class="price-tag mt-2 font-bold">' + rp.price + ' AED</p>';
                                ...
                                <button class="cart-btn ml-2 shadow-sm relative group" onclick="window.addToCart(event, '${rp.id}')">
