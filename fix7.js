...
// 1. Convert the Reviews IIFE to use empty innerHTML.
...
if (rStart !== -1) {
    ...
    content = content.substring(0, rStart) + '<div id="reviews-section-container"></div>' + content.substring(rEnd);
}

// 2. Convert the Recommendations IIFE to use empty innerHTML.
...
if (cStart !== -1) {
    ...
    content = content.substring(0, cStart) + '<div id="recommendations-section-container"></div>' + content.substring(cEnd);
}

// 3. Now we inject the actual logic and `isUpdate` condition before appMain.innerHTML.
// The file starts with `export function renderProductDetailView...`
// Then it has `/* isUpdate block removed */`. Let's just replace `/* isUpdate block removed */` with our logic!
const logic = `
...
// Remove everything from line 20 (const reviewCount) down to the start of appMain.innerHTML
const startRemove = content.indexOf('const reviewCount = reviews ? reviews.length : 0;');
const endRemove = content.indexOf('appMain.innerHTML = `');

content = content.substring(0, startRemove) + logic + '\n\n    ' + content.substring(endRemove);
